...
export const contextKitCaseStudy: CaseStudyContent = {
  ...
  understanding: {
    ...
    keyInsights: [
      ...
      'File paths beat database IDs - Filesystem references feel more concrete',
      'Explicit relationships matter - Don\'t make agents infer connections',
    ],
  },

  solution: {
    title: 'The _project.yml Context Map',
    content: [
      ...
      'I discovered that agents were repeating similar context patterns, so the anchor system lets me define something like &tech-stack once and reference it multiple times without bloating token count. This reduced my _project.yml from ~1000 lines to ~300 lines while maintaining full information.',
      '',
      ...
      'The _project.yml is the foundation, but the full context-kit includes a service-oriented architecture with unified React 19 dashboard (port 42001), knowledge graph backend API with SQLite+FTS5 storage (port 42003), centralized logging service, and MCP integration for persistent AI memory across sessions.',
      '',
